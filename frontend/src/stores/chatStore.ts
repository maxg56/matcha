    ...
    (set, get) => ({
      ...
      sendReactionWebSocket: (messageId: number, emoji: string, action: 'add' | 'remove') => {
        ...
        if (!messageId || messageId <= 0) {
          console.error('ChatStore: Invalid message ID for reaction:', messageId);
          return;
        }

        // Check if this looks like a temporary ID (generated by Date.now())
        // Database IDs are typically smaller sequential numbers
        const now = Date.now();
        const oneHourAgo = now - (60 * 60 * 1000);
        if (messageId > oneHourAgo) {
          console.warn('ChatStore: Message ID appears to be temporary (Date.now()), skipping reaction:', messageId);
          console.warn('ChatStore: This might indicate the message hasn\'t been persisted to database yet');
          return;
        }

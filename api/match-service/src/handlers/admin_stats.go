...
func GetAdminStatsHandler(c *gin.Context) {
	...
	if err := db.Table("users").Count(&stats.TotalUsers).Error; err != nil {
		utils.RespondError(c, "Failed to get users count: "+err.Error(), http.StatusInternalServerError)
		return
	}
	
	// Get total matches count  
	if err := db.Table("matches").Count(&stats.TotalMatches).Error; err != nil {
		utils.RespondError(c, "Failed to get matches count: "+err.Error(), http.StatusInternalServerError)
		return
	}
	
	// Get total interactions count
	if err := db.Table("user_interactions").Count(&stats.TotalInteractions).Error; err != nil {
		utils.RespondError(c, "Failed to get interactions count: "+err.Error(), http.StatusInternalServerError)
		return
	}
	...
	topUsersQuery := `
		...
		FROM users u
		LEFT JOIN matches m ON (u.id = m.user_id OR u.id = m.matched_user_id)  
	...
	if err := db.Raw(topUsersQuery).Scan(&stats.TopUsers).Error; err != nil {
		utils.RespondError(c, "Failed to get top users: "+err.Error(), http.StatusInternalServerError)
		return
	}
	...
	dailyStatsQuery := `
		SELECT 
			...
			COUNT(DISTINCT CASE WHEN 'm' = 'matches' THEN id END) as new_matches,
			COUNT(DISTINCT CASE WHEN 'm' = 'interactions' THEN id END) as new_interactions,
			COUNT(DISTINCT user_id) as active_users
		FROM (
			SELECT id, user_id, created_at, 'matches' as m FROM matches WHERE created_at >= CURRENT_DATE - INTERVAL '30 days'
			UNION ALL
			SELECT id, user_id, created_at, 'interactions' as m FROM user_interactions WHERE created_at >= CURRENT_DATE - INTERVAL '30 days'
		) combined
	...
	if err := db.Raw(dailyStatsQuery).Scan(&stats.DailyStats).Error; err != nil {
		utils.RespondError(c, "Failed to get daily stats: "+err.Error(), http.StatusInternalServerError)
		return
	}
	...
	if err := db.Raw(interactionStatsQuery).Scan(&stats.InteractionTypes).Error; err != nil {
		utils.RespondError(c, "Failed to get interaction stats: "+err.Error(), http.StatusInternalServerError)
		return
	}
...
func GetUserStatsHandler(c *gin.Context) {
	...
	if err := db.Raw(userStatsQuery, userID).Scan(&userStats).Error; err != nil {
		utils.RespondError(c, "Failed to get user stats: "+err.Error(), http.StatusInternalServerError)
		return
	}
	
	if userStats.UserID == 0 {
		utils.RespondError(c, "User not found", http.StatusNotFound)
		return
	}
	
	utils.RespondSuccess(c, http.StatusOK, userStats)
}
...
func GetMatchTrendsHandler(c *gin.Context) {
	...
	if err := db.Raw(query).Scan(&trends).Error; err != nil {
		utils.RespondError(c, "Failed to get match trends: "+err.Error(), http.StatusInternalServerError)
		return
	}

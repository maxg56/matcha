...
def get_user_media(target_user_id):
    ...
    try:
        ...
        logger.info(f"Returned {len(media_list)} public media files for user {target_user_id}")
        return respond_success({
            "media": media_list,
...
def set_profile_image():
    ...
    try:
        ...
        if image.user_id != user_id:
            return respond_error("Access denied: you can only set your own images as profile", 403)
        
        # Set as profile image
        image.set_as_profile()
        
        logger.info(f"Profile image set successfully: image {image_id} for user {user_id}")
        return respond_success({
            "id": image.id,
            "filename": image.filename,
            "is_profile": image.is_profile
